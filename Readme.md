...
![](/images/htop.png)
![](/images/gra1.png)
## Видно, что сначала при увеличении количества корзин производительность растет, так как время на линейный поиск внутри корзины уменьшается. Достигает минимума в районе ~5000 а потом растет.Уменьшение производительности при увеличении размера таблицы связано с тем, что приходится обращаться в отдаленные друг от друга участки памяти(так ведь???) + таблица все меньше помещается в кеш.
...
# 3 График с обычным приоритетом 
## `./table` 
![](/images/gra3.png)
...
# Ничего не меняется, скорее всего потому, что в это время никаких дополнительных нагрузок на процессор не было, поэтому попробуем запустить с высшим и низшим приоритетом, дополнительно запустив `stress --cpu 2`
...
# Вывод: когда на процессоре не запущено никаких дополнительных задач, то особо нет разницы запускать на выделенном ядре/высшим/низшим приоритетом. Когда ядро дополнительно загружено другими задачами, то приоритет не дает большого вклада в точность измерений.
